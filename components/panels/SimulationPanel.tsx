...
export function SimulationPanel({
  ...
  return (
    <div className="flex w-[380px] flex-col border-l border-stone-200 bg-[#faf9f7]">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
        ...
        <div className="flex items-center gap-1.5">
          {isComplete && impacts.length > 0 && (
            <button
              onClick={onExport}
              className="rounded-md bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-700 transition-colors hover:bg-slate-200"
            >
              Export .md
            </button>
          )}
      ...
      {/* Change description */}
      <div className="border-b border-stone-100 bg-stone-50/50 px-4 py-2.5">
        <p className="text-[10px] font-medium uppercase tracking-wider text-stone-500">
          Change
        </p>
        <p className="mt-0.5 text-xs text-slate-700">{changeDescription}</p>
        ...
        <div className="mt-2 space-y-2">
          ...
          {impact.regulationReference && (
            <div className="flex items-start gap-1.5">
              <span className="mt-px text-[10px] text-slate-400">§</span>
