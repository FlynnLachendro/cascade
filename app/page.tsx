  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="mx-auto max-w-3xl px-8 pb-20 pt-16 text-center">
        ...
        <img src="/cascade-logo.png" alt="Cascade" className="mx-auto mb-8 h-36 w-36" />
        ...
        <p className="mx-auto mt-5 max-w-lg text-lg text-[#5a6577]">
          Simulate a process change and instantly trace cascading impacts across
          SOPs, training, validation, and regulatory submissions — with
      ...
      <section className="border-t border-[#e2e6ea] bg-[#f4f5f7] px-8 py-20">
        <div className="mx-auto max-w-4xl">
          <h2 className="text-center text-2xl font-bold text-[#1a2332]">
            Three steps to trace your impact
          </h2>
          <p className="mx-auto mt-3 max-w-lg text-center text-sm text-[#5a6577]">
      ...
      <section className="border-t border-[#e2e6ea] bg-[#f4f5f7] px-8 py-10">
        <div className="mx-auto max-w-4xl text-center">
          <p className="text-[11px] font-medium uppercase tracking-wider text-[#8b95a5]">
            Built with
          </p>
          <div className="mt-4 flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
            {["Next.js", "TypeScript", "GraphQL", "React Flow", "Turso", "OpenRouter"].map((tech) => (
              <span key={tech} className="text-sm font-medium text-[#5a6577]">
                {tech}
              </span>
            ))}
          </div>
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div className="text-center">
      <p className="text-2xl font-bold text-[#1a3a6b]">{value}</p>
      <p className="mt-1 text-xs text-[#5a6577]">{label}</p>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-[#e2e6ea] bg-white p-6">
      <div className="flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#f0f3f8]">
          {icon}
        </div>
        <span className="text-xs font-bold text-[#1a3a6b]/30">{number}</span>
      </div>
      <h3 className="mt-4 text-base font-semibold text-[#1a2332]">{title}</h3>
...
function GraphIcon() {
  return (
    <svg className="h-5 w-5 text-[#1a3a6b]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
